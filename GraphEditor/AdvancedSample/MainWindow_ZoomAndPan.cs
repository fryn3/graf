...
namespace SampleCode
{
	...
	public partial class MainWindow
	{
		...
		private NodeViewModel sourceConnectionNode;
		...
		/// </summary>
		private void CreateConnection_Executed(object sender, ExecutedRoutedEventArgs e)
		{
			switch (networkControl.SelectedNodes.Count)
			{
				case 1:
					sourceConnectionNode = networkControl.SelectedNodes[0] as NodeViewModel;
					break;
				case 2:
					var node1 = networkControl.SelectedNodes[0] as NodeViewModel;
					var node2 = networkControl.SelectedNodes[1] as NodeViewModel;
					ViewModel.ConnectNodes(node1, node2);
					break;
				default:
					MessageBox.Show("Для добавления перехода между состояниями выделите первое состояние перехода, " +
									"затем нажмите на кнопку \"Добавить переход\", " +
									"и, наконец, выделите второе состояние.",
									"Порядок действий", MessageBoxButton.OK, MessageBoxImage.Information);
					break;
			}
		}
		...
		//
		private void networkControl_OnSelectionChanged(object sender, SelectionChangedEventArgs e)
		{
			if (sourceConnectionNode != null && networkControl.SelectedNodes.Count > 0)
			{
				var node2 = networkControl.SelectedNodes[0] as NodeViewModel;
				ViewModel.ConnectNodes(sourceConnectionNode, node2);
				sourceConnectionNode = null;
			}
		}
		...
		private void ConnectNodes(NodeViewModel target)
		{
			this.ViewModel.ConnectSelectedNodes(target);
		}
